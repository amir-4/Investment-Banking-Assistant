...
@tool
def get_latest_financial_metrics(cik: str) -> dict:
    """
    Retrieve a small, normalized set of the latest annual financial
    metrics from SEC Company Facts.
    """

    cik_clean = str(cik).strip().upper().replace("CIK", "").strip()
    cik_clean = cik_clean.zfill(10)
    ...
    def get_latest_annual_fact(tag_candidates):
    ...
    for metric_name, tag_candidates in requested_metrics.items():
        fact = get_latest_annual_fact(tag_candidates)

        if fact is not None:
            metrics[metric_name] = fact
